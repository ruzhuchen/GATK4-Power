...
/usr/bin/time -v -o time_bwa_jemalloc.log taskset -c 0-159 /gpfs/gpfs_4mb/rchen/Power9/BWA/bwa-0.7.17/bwa mem -t 160 -Ma \
     -R '@RG\tID:sample_lane\tSM:sample\tPL:illumina\tLB:sample\tPU:lane' \
     $ref $input1 $input2 | samtools view -bS - -@ 20 | samtools sort - -@ 40 -n -m 4G -T $input1 -o $output
...
/usr/bin/time -v -o time_gatkBaseRecalibrator.log taskset -c 0-159:4 gatk --java-options "-Xmx8G -XX:+UseParallelGC -XX:ParallelGCThreads=4" \
...
# VARIANT CALLING optimized for 40 cores Power9 ##########
